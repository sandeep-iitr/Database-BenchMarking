...
static FeatureCollection createNewFeatures(SimpleFeatureType simpleFeatureType, String uuid2, JsonArray data) {
		...
		try {
		  ...
		  for (int i = 0; i < data.size(); i++) {
		    ...
		    String uuid = datum.getString("uuid");
			long timestamp = datum.getLong("timestamp");//timestamp is in milliseconds
			String value = datum.getString("value");
			Date date= new Date(timestamp);
			String geometryType = datum.getString("geometryType").toLowerCase();  
		    JsonArray coordinates=datum.getJsonArray("coordinates");
		    
		    if (geometryType.equals("point")) {
		      Double lat = coordinates.getJsonArray(0).getDouble(0);
		      Double lng = coordinates.getJsonArray(0).getDouble(1);
	...
	public JsonArray Query_Box_Lat_Lng_Time_Range(String uuid, Double lat_min, Double lat_max, Double lng_min, Double lng_max,
